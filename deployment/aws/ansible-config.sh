...
createSSHConfigFile()
{
    ...
    case $PLATFORM_OPTION in
        3)  BASTION_IP=$(terraform output -state=../windfire-restaurants-devops/aws/SingleZone/terraform.tfstate bastion-public_ip)
            BASTION_HOSTNAME=$(terraform output -state=../windfire-restaurants-devops/aws/SingleZone/terraform.tfstate bastion-public_dns)
            ;;
        4)  BASTION_IP=$(terraform output -state=../windfire-restaurants-devops/aws/MultiZone/terraform.tfstate bastion-public_ip)
    ...
    #BASTION_IP=$(terraform output -state=../windfire-restaurants-devops/aws/SingleZone/terraform.tfstate bastion-public_ip)
    #BASTION_HOSTNAME=$(terraform output -state=../windfire-restaurants-devops/aws/SingleZone/terraform.tfstate bastion-public_dns)
